...
and transl_type_aux env policy mode styp =
  ...
  | Ptyp_class(lid, stl) ->
      ...
      let ty = match get_desc ty with
        Tvariant row ->
          ...
          let more =
            if policy = Univars then new_pre_univar Type_layout.value
            else newvar Type_layout.value
          in
          let row =
  ...
  | Ptyp_variant(fields, closed, present) ->
      ...
      in
      let more =
        if Btype.static_row (make_row (newvar Type_layout.value))
        then newty Tnil
        else if policy = Univars then new_pre_univar Type_layout.value
        else newvar Type_layout.value
      in
...
and transl_fields env policy o fields =
  ...
  let ty_init =
     match o, policy with
     | Closed, _ -> newty Tnil
     | Open, Univars -> new_pre_univar Type_layout.value
     | Open, _ -> newvar Type_layout.value in
...
let globalize_used_variables env fixed =
  ...
  TyVarMap.iter
    (fun name (ty , loc) ->
      let v = new_global_var Type_layout.any in
  ...
  let univs =
    List.fold_left
      (fun acc v ->
        match get_desc v with
          Tvar { name; layout } when get_level v = Btype.generic_level ->
            set_type_desc v (Tunivar { name; layout }); v :: acc
        | _ -> acc)
      [] !pre_univars
  in
  make_fixed_univars typ.ctyp_type;
